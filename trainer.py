...
with tf.device('/device:GPU:0'):
    ...
    #x = nm.Conv2Plus1D(filters=16, kernel_size=(3, 7, 7), padding='same')(x)
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.ReLU()(x)
    newH = HEIGHT // 2  # 256 -> 128
    newW = WIDTH // 2
    x = ResizeVideo(newH, newW)(x)
    ...
    # Visualize the model
    tf.keras.utils.plot_model(model, expand_nested=True, dpi=60, show_shapes=True, to_file=SOURCE_FOLDER + '/model.png')
